...
import { Upload, FileText, ImageIcon, X, AlertCircle, Check, Eye } from "lucide-react"
import { z } from "zod"
...
export function FileUpload({ onExtractedData, maxGrade }: FileUploadProps) {
    ...
    try {
      ...
      if (file.type === "application/pdf") {
        ...
        try {
          ...
          console.error("Image processing failed, falling back to PDF processing:", err)
          // If image processing fails, try to process as PDF if possible
    ...
    try {
      ...
      if (courses.length > 0) {
        ...
        toast({
          ...
          description: `Successfully extracted ${courses.length} courses.`,
        })
      } else {
        toast({
          title: "No Data Found",
  ...
  const processImage = async (file: File) => {
    try {
      ...
      // Create an image data URL for Tesseract.js to process
      ...
      // Create worker with the correct API for v5
      ...
      // Extract course data
      const courses = extractCourseData(data.text, maxGrade, false) // Pass false to extract grades if available
      ...
      if (courses.length > 0) {
        ...
        toast({
          title: "No Data Found",
          description: "Could not find course data in the image. Please check the format and try again.",
          variant: "destructive",
        })
      }
    } catch (error) {
      ...
      toast({
        ...
        description: "We couldn't process the image. Try uploading a clearer image or a PDF instead.",
        variant: "destructive",
      })
          ...
          {!file ? (
            <>
              ...
              <div className="space-y-1">
                ...
                <p className="text-xs text-muted-foreground">PDF or image files only (max 5MB)</p>
              </div>
